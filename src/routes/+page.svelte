...
<div class="mx-auto min-h-screen max-w-md rounded-3xl bg-gray-100 p-4 shadow-lg">
  ...
  <div class="mb-6 flex items-center gap-2">
    ...
    <!-- <button class="rounded-xl bg-gray-200 p-2" aria-label="filters">
  ...
  <div class="grid grid-cols-2 gap-4">
    {#each filtered() as item}
      <div class={`rounded-xl p-4 text-center`} style:background={Gradients[item.elements[0]][0]}>
        <img
          src={`/img/${item.number}.png`}
          alt={item.name}
          class="mx-auto mb-2 h-20 w-20 duration-100 ease-in hover:scale-120"
        />
