...
>
  ...
  <div class="-mt-4 rounded-3xl bg-white p-6 shadow-lg">
    {#if getSource(geekmon) !== "base" && getSource(geekmon) !== "catarse"}
      <img class="mx-auto max-h-20" src="{base}/img/logos/{getSource(geekmon)}.png" alt="" />
    {/if}
    ...
    <div class="mt-3 flex gap-1">
      {#each geekmon.habitat as habitat (habitat)}
        <img src={`${base}/img/habitats/${habitat}.png`} alt={habitat} class="size-20" />
      {/each}
    </div>
